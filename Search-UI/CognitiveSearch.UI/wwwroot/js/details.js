...
function ShowDocument(id, index) {
    $.post('/home/getdocumentbyid',
        ...
        function (data) {
            ...
            var fullFileContainerHTML = GetFileHTML(data, result, true);
            var summaryContainerHTML = GetSummaryOpenAI(result);
            ...
            pivotsHTML += '          <div id="file-full" class="ms-Pivot-content" data-content="fullfile"> ';
            pivotsHTML += '                 <div id="full-file-viewer" style="height: 100%;"></div> ';
            pivotsHTML += '             </div> ';

            pivotsHTML += '          <div id="summary-pivot" class="ms-Pivot-content" data-content="summary"> ';
            pivotsHTML += '                 <div id="summary-viewer" style="height: 100%;"></div> ';
            pivotsHTML += '             </div> ';
            ...
            $('#full-file-viewer').html(fullFileContainerHTML);
            

            if (transcriptContainerHTML !== null) {
                $('#transcript-viewer').html(transcriptContainerHTML);
                $('#summary-viewer').html(summaryContainerHTML);
            }
            ...
            if (transcriptContainerHTML !== null) {
                ...
                pivotLinksHTML += '<li id="summary-pivot-link" class="ms-Pivot-link" data-content="summary" title="Summary" tabindex="1" onclick="SearchBarVisibility(true)">Summary</li>';
            }
            ...
            if (result.geoLocation !== null) {
                ...
                pivotLinksHTML += '<li id="full-file-pivot-link" class="ms-Pivot-link" data-content="fullfile" title="Full File" tabindex="1" onclick="SearchBarVisibility(' + fileSearchBar + ')">' + 'Full ' + fileName + '</li>';
...
function GetFileHTML(data, result, fullFile= false) {
...
function GetSummaryOpenAI(result) {
    ...
    if (result.summary) {
        if (result.summary.length > 0) {
            full_content = htmlDecode(result.summary.trim());
        }
    }
    else {
        // otherwise, let's try getting the content -- although it won't have any image data.
    ...
    transcriptContainerHTML = '<div style="overflow-x:auto;"><table class="table table-hover table-striped table-bordered"><thead><tr><th>Original Content</th></tr></thead>';
    transcriptContainerHTML += '<tbody>';
    transcriptContainerHTML += '<tr><td class="wrapword"><pre id="summary-viewer-pre">' + full_content + '</pre></td></tr>';
    transcriptContainerHTML += '</tbody>';
    transcriptContainerHTML += '</table></div>';
